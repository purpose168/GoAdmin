...
//	Chi 是一个轻量级、可组合的 HTTP 路由器，专注于 HTTP 服务
//	该适配器实现了 GoAdmin 的适配器接口，将 Chi 的请求和响应对象转换为 GoAdmin 的上下文
//	支持 Chi 的路由系统，包括路由参数、中间件链、请求处理等
//	支持 Cookie 认证、PJAX 请求、多语言等 GoAdmin 核心功能
//
// 核心概念:
//
//	适配器模式: 将 GoAdmin 管理后台集成到 Chi 框架中，实现框架无关性
//	上下文转换: 将 Chi 的 http.Request 和 http.ResponseWriter 转换为 GoAdmin 的上下文
//	路由集成: 将 GoAdmin 的路由注册到 Chi 的路由系统中，支持路由参数和中间件
//	中间件链: 支持 Chi 的中间件链，实现请求预处理和后处理
...
// 技术栈:
//
//	Chi: 轻量级、可组合的 HTTP 路由器，提供路由、中间件、上下文等功能
//	GoAdmin: Go 后台管理框架，提供管理后台、表单、表格、权限等功能
//	Go 标准库: net/http、net/url、regexp、strings 等，提供 HTTP、URL、正则表达式、字符串处理等功能
//
// 数据库支持:
//
...
//	    chiAdapter := chi.NewChiAdapter()
//	    chiAdapter.Use(r, []plugins.Plugin{adminPlugin})
//
//	    // 4. 启动 Chi 服务
...
// 注意事项:
//
//	- Chi 版本: 建议使用 Chi v5.x 或更高版本
//	- 路由参数格式: Chi 的路由参数格式为 :param，会自动转换为 Go 标准格式 {param}
...
import (
	...
	"regexp"   // 正则表达式，提供正则表达式匹配和替换功能
	"strings"  // 字符串操作，提供字符串处理和转换功能

	"github.com/go-chi/chi"                                        // Chi Web 框架，提供路由、中间件、上下文等功能
	...
	cfg "github.com/purpose168/GoAdmin/modules/config"             // GoAdmin 配置模块，提供配置管理和 URL 生成功能
...
// 它作为 Chi 框架和 GoAdmin 管理后台之间的桥梁，实现了适配器接口的所有方法
// 嵌入了 adapter.BaseAdapter 以获得基础适配器功能（如 GetUser、GetUse、GetContent 等）
// ctx 字段存储当前的 Chi 上下文，用于访问请求和响应对象
// app 字段存储 Chi 路由器实例，用于注册路由和处理器
type Chi struct {
	adapter.BaseAdapter          // 嵌入基础适配器，获得基础适配器功能
	ctx                 Context  // 当前 Chi 上下文，包含请求和响应对象
	app                 *chi.Mux // Chi 路由器实例，用于注册路由和处理器
}
...
//	适配器注册: 将 Chi 适配器注册到 GoAdmin 引擎中，使 GoAdmin 能够使用 Chi 框架
//	支持多个适配器: 可以同时注册多个适配器，GoAdmin 会根据配置选择使用哪个适配器
//	线程安全: 注册过程是线程安全的，可以在多个 goroutine 中并发调用
//
// 说明:
//
//	该函数使用 engine.Register 方法将 Chi 适配器注册到 GoAdmin 引擎中
//	Register 方法接收一个适配器接口，这里传入 new(Chi) 创建 Chi 适配器实例
//	注册后，GoAdmin 引擎会保存 Chi 适配器的引用，用于后续的路由注册和请求处理
...
//   - 该函数支持并发调用，但通常只执行一次
func init() {
	engine.Register(new(Chi)) // 将 Chi 适配器注册到 GoAdmin 引擎中
}
...
//
// 参数:
//   - ctx: 上下文接口，通常为 Context 类型，包含请求和响应对象
//
// 返回值:
...
func (ch *Chi) User(ctx interface{}) (models.UserModel, bool) {
	return ch.GetUser(ctx, ch) // 调用基础适配器的 GetUser 方法，传入上下文和适配器实例
}

// Use 实现了 Adapter.Use 方法
// 该方法用于将插件注册到 Chi 应用中，用于扩展 GoAdmin 的功能
//
// 参数:
//   - app: 应用接口，必须为 *chi.Mux 类型，表示 Chi 路由器实例
...
// 功能特性:
//
//	注册插件: 将插件注册到 Chi 应用中，用于扩展 GoAdmin 的功能
...
func (ch *Chi) Content(ctx interface{}, getPanelFn types.GetPanelFn, fn context.NodeProcessor, btns ...types.Button) {
	ch.GetContent(ctx, getPanelFn, ch, btns, fn) // 调用基础适配器的 GetContent 方法，传入上下文、获取面板函数、适配器实例、导航按钮和节点处理器
}
...
// 说明:
//
//	该类型定义了处理函数的签名，用于在 Chi 路由中使用 GoAdmin 的处理函数
//	处理函数接收 Chi 上下文，可以访问请求和响应对象
//	处理函数返回面板和错误，面板会被渲染为 HTML 响应
//	处理函数可以访问数据库、调用 API、执行业务逻辑等
//	该类型通常与 Content 辅助函数一起使用，将 HandlerFunc 转换为 http.HandlerFunc
//
// 使用示例:
//
//	func myHandler(ctx Context) (types.Panel, error) {
//	    // 访问数据库
...
//
// 返回值:
//   - http.HandlerFunc: HTTP 处理器函数，可以在 Chi 路由中使用
//
// 功能特性:
//
//	转换 HandlerFunc 为 Handler: 将 GoAdmin 的处理函数转换为 Chi 的处理器函数
//	简化集成: 简化了 GoAdmin 与 Chi 的集成过程，无需手动处理上下文转换
//	支持在 Chi 路由中使用 GoAdmin 处理函数: 可以在 Chi 路由中直接使用 GoAdmin 的处理函数
//	支持自定义处理逻辑: 可以在处理函数中实现自定义的业务逻辑
//
// 说明:
//
//	该函数接收 HandlerFunc 类型的处理函数，返回 http.HandlerFunc 类型的处理器函数
//	处理器函数内部创建 Chi 上下文，包含请求和响应对象
//	然后调用 GoAdmin 的 engine.Content 方法，传入上下文和处理函数
//	engine.Content 方法会调用处理函数，获取面板并渲染为 HTML 响应
//	该函数通常在路由注册时使用，将 GoAdmin 的处理函数注册到 Chi 路由中
//
// 使用示例:
//
//	// 定义处理函数
...
//	}
//
//	// 使用 Content 辅助函数创建 Chi 处理器
//	r := chi.NewRouter()
//	r.Get("/admin", Content(myHandler))
//
// 注意事项:
//
//   - 处理函数不能为空，必须返回有效的面板
...
func Content(handler HandlerFunc) http.HandlerFunc {
	return func(writer http.ResponseWriter, request *http.Request) { // 返回 http.HandlerFunc 类型的处理器函数
		ctx := Context{ // 创建 Chi 上下文
			Request:  request, // 设置请求对象
			Response: writer,  // 设置响应对象
		}
		engine.Content(ctx, func(ctx interface{}) (types.Panel, error) { // 调用 GoAdmin 的 engine.Content 方法，传入上下文和处理函数
			return handler(ctx.(Context)) // 调用处理函数，传入 Chi 上下文
		})
	}
}

// SetApp 实现了 Adapter.SetApp 方法
// 该方法用于设置 Chi 路由器实例到适配器中，用于后续的路由注册和请求处理
//
// 参数:
//   - app: 应用接口，必须为 *chi.Mux 类型，表示 Chi 路由器实例
//
...
// 功能特性:
//
//	设置 Chi 路由器实例: 将 Chi 路由器实例设置到适配器中，用于后续的路由注册和请求处理
//	支持类型检查: 使用类型断言检查参数类型，确保参数为 *chi.Mux 类型
//	支持错误处理: 如果参数类型不正确，返回错误信息
//
// 说明:
//
//	该方法使用类型断言检查 app 参数是否为 *chi.Mux 类型
//	如果类型断言成功，将 Chi 路由器实例存储到适配器的 app 字段中
//	如果类型断言失败，返回错误信息，表示参数类型错误
//	该方法通常在 Use 方法中被调用，用于设置 Chi 路由器实例
//	设置后，适配器就可以使用 Chi 路由器实例注册路由和处理器了
//
// 注意事项:
//
//   - app 参数必须为 *chi.Mux 类型，否则会返回错误
//   - 该方法应该在 Use 方法之前调用，确保在注册插件前设置 Chi 路由器实例
//   - 该方法只能调用一次，重复调用会覆盖之前的设置
//   - 该方法支持并发调用，但通常只调用一次
//   - 返回错误后适配器无法正常工作，需要检查参数类型
func (ch *Chi) SetApp(app interface{}) error {
	var (
		eng *chi.Mux // Chi 路由器实例
		ok  bool     // 类型断言结果
	)
	// 使用类型断言检查 app 是否为 *chi.Mux 类型
	// ok 为 true 表示断言成功，eng 为转换后的值
	if eng, ok = app.(*chi.Mux); !ok { // 如果类型断言失败
		return errors.New("chi 适配器 SetApp: 参数类型错误") // 返回错误信息
	}
	ch.app = eng // 将 Chi 路由器实例存储到适配器的 app 字段中
	return nil   // 返回 nil，表示设置成功
}

// AddHandler 实现了 Adapter.AddHandler 方法
// 该方法用于向 Chi 路由器添加路由处理器，将 GoAdmin 的路由注册到 Chi 的路由系统中
//
// 参数:
...
// 功能特性:
//
//	注册路由: 将 GoAdmin 的路由注册到 Chi 的路由系统中
//	支持多种 HTTP 方法: 支持 GET、POST、PUT、DELETE、HEAD、OPTIONS、PATCH 等 HTTP 方法
//	支持路由参数: 支持路由参数（如 :id、:name 等），会自动转换为 Go 标准格式 {param}
//	支持处理器链: 支持处理器链（中间件和处理器），实现请求预处理和后处理
//	支持上下文转换: 将 Chi 上下文转换为 GoAdmin 上下文，实现框架无关性
//	支持响应头复制: 将 GoAdmin 响应头复制到 Chi 响应中，确保响应头正确传递
//	支持响应体写入: 将 GoAdmin 响应体写入到 Chi 响应中，确保响应体正确传递
//
// 说明:
//
//	该方法调用 Chi 路由器的路由注册函数，注册路由处理器
//	路由处理器接收 Chi 上下文，处理 HTTP 请求和响应
//	首先使用正则表达式将 Chi 的路由参数格式 :param 转换为 Go 标准格式 {param}
...
//	1. 如果路径以 / 结尾，移除末尾的 /
//	2. 创建 GoAdmin 上下文
//	3. 获取 Chi 的路由参数并转换为 URL 查询参数
//	4. 执行处理器链
//	5. 将 GoAdmin 响应头复制到 Chi 响应中
//	6. 设置响应状态码
//	7. 将响应体写入 Chi 响应
...
func (ch *Chi) AddHandler(method, path string, handlers context.Handlers) {
	...
	// 如果路径以 // 开头，移除第一个 /
	if len(url) > 1 && url[0] == '/' && url[1] == '/' { // 检查路径前两个字符是否为 //
		url = url[1:] // 移除第一个 /
	}

	// 根据方法类型获取对应的路由注册函数
	getHandleFunc(ch.app, strings.ToUpper(method))(url, func(w http.ResponseWriter, r *http.Request) { // 调用 getHandleFunc 获取路由注册函数，注册路由处理器

		// 如果路径以 / 结尾，移除末尾的 /
		if r.URL.Path[len(r.URL.Path)-1] == '/' { // 检查路径最后一个字符是否为 /
			r.URL.Path = r.URL.Path[:len(r.URL.Path)-1] // 移除末尾的 /
		}

		// 创建 GoAdmin 上下文
		ctx := context.NewContext(r) // 创建 GoAdmin 上下文，传入 HTTP 请求

		// 获取 Chi 的路由参数并转换为 URL 查询参数
		params := chi.RouteContext(r.Context()).URLParams // 获取 Chi 的路由参数

		for i := 0; i < len(params.Values); i++ { // 遍历路由参数
			if r.URL.RawQuery == "" { // 如果 URL 查询参数为空
				r.URL.RawQuery += strings.ReplaceAll(params.Keys[i], ":", "") + "=" + params.Values[i] // 添加第一个查询参数，格式为 ?param=value
			} else { // 如果 URL 查询参数不为空
				r.URL.RawQuery += "&" + strings.ReplaceAll(params.Keys[i], ":", "") + "=" + params.Values[i] // 添加后续查询参数，格式为 &param=value
			}
		}
		...
		for key, head := range ctx.Response.Header { // 遍历 GoAdmin 响应头
			w.Header().Set(key, head[0]) // 将响应头设置到 Chi 响应中
		}
		// 将响应体写入 Chi 响应
		if ctx.Response.Body != nil { // 如果 GoAdmin 响应体不为空
			...
			w.WriteHeader(ctx.Response.StatusCode) // 设置 Chi 响应的状态码为 GoAdmin 响应的状态码
			_, _ = w.Write(buf.Bytes())            // 将缓冲区数据写入 Chi 响应
		} else { // 如果 GoAdmin 响应体为空
			w.WriteHeader(ctx.Response.StatusCode) // 只设置 Chi 响应的状态码
		}
	})
}
...
//
// 参数:
//   - pattern: 路由模式，如 "/admin"、"/admin/users/:id" 等
//   - handlerFn: 处理器函数，类型为 http.HandlerFunc
//
// 返回值:
//   - *chi.RouteInfo: 路由信息指针，包含路由的详细信息
//
// 功能特性:
//
//	定义 Chi 路由注册函数的签名: 定义了 Chi 路由注册函数的参数和返回值类型
//	支持 GET、POST 等 HTTP 方法: 支持所有 HTTP 方法，如 GET、POST、PUT、DELETE 等
//	支持路由信息返回: 返回路由信息指针，包含路由的详细信息
//
// 说明:
//
//	该类型定义了 Chi 路由注册函数的签名，用于在 getHandleFunc 函数中使用
//	路由注册函数接收路由模式和处理器函数，返回路由信息指针
//	路由模式可以包含路由参数，如 "/admin/users/:id"
//	处理器函数用于处理请求，返回响应
//	该类型通常与 getHandleFunc 函数一起使用，根据方法名称返回对应的路由注册函数
//
// 注意事项:
//
//   - 路由模式不能为空，必须以 "/" 开头
//   - 处理器函数不能为空，必须实现 http.HandlerFunc 接口
...
//
// 参数:
//   - eng: Chi 路由器实例，类型为 *chi.Mux
//   - method: HTTP 方法名称，如 "GET"、"POST"、"PUT"、"DELETE" 等
//
// 返回值:
//   - HandleFun: 对应的路由注册函数，类型为 HandleFun
//
// 功能特性:
//
//	根据方法名称返回对应的路由注册函数: 将字符串方法名转换为 Chi 的路由注册函数
//	支持所有 HTTP 方法: 支持 GET、POST、PUT、DELETE、HEAD、OPTIONS、PATCH 等 HTTP 方法
//	支持错误处理: 如果方法名称不支持，会 panic
//
// 说明:
//
//	该函数接收 Chi 路由器实例和 HTTP 方法名称，返回对应的路由注册函数
//	使用 switch 语句根据方法名称返回对应的路由注册函数
//	支持的方法包括 GET、POST、PUT、DELETE、HEAD、OPTIONS、PATCH
//	如果方法名称不支持，会 panic，表示方法名称错误
//	该函数通常在 AddHandler 方法中被调用，用于获取路由注册函数
//
// 注意事项:
//
//   - method 参数必须为大写，如 "GET"、"POST" 等
//   - method 参数必须是支持的 HTTP 方法之一，否则会 panic
//   - 该函数支持并发调用，但通常只调用一次
func getHandleFunc(eng *chi.Mux, method string) HandleFun {
	switch method { // 根据方法名称返回对应的路由注册函数
	case "GET": // 如果方法为 GET
		return eng.Get // 返回 GET 路由注册函数
	case "POST": // 如果方法为 POST
		return eng.Post // 返回 POST 路由注册函数
	case "PUT": // 如果方法为 PUT
		return eng.Put // 返回 PUT 路由注册函数
	case "DELETE": // 如果方法为 DELETE
		return eng.Delete // 返回 DELETE 路由注册函数
	case "HEAD": // 如果方法为 HEAD
		return eng.Head // 返回 HEAD 路由注册函数
	case "OPTIONS": // 如果方法为 OPTIONS
		return eng.Options // 返回 OPTIONS 路由注册函数
	case "PATCH": // 如果方法为 PATCH
		return eng.Patch // 返回 PATCH 路由注册函数
	default: // 如果方法不支持
		panic("错误的 HTTP 方法") // panic，表示方法名称错误
	}
}
...
//
// 字段说明:
//   - Request: HTTP 请求对象，包含请求的所有信息（如 URL、方法、头、体等）
//   - Response: HTTP 响应对象，用于写入响应数据（如状态码、头、体等）
//
// 功能特性:
//
//	封装请求和响应对象: 将 Chi 的请求和响应对象封装为一个结构体，便于传递
//	支持请求处理: 可以访问请求的所有信息，如 URL、方法、头、体等
//	支持响应写入: 可以写入响应数据，如状态码、头、体等
//	支持上下文传递: 可以在适配器中传递请求和响应信息
//
// 说明:
//
//	该结构体封装了 Chi 的 http.Request 和 http.ResponseWriter 对象
//	Request 字段包含请求的所有信息，如 URL、方法、头、体等
//	Response 字段用于写入响应数据，如状态码、头、体等
//	该结构体通常在 SetContext 方法中使用，用于设置当前请求的上下文
//	设置后，适配器就可以使用该上下文访问请求和响应对象了
//
// 注意事项:
//
//   - Request 字段不能为空，必须包含有效的 HTTP 请求对象
//   - Response 字段不能为空，必须包含有效的 HTTP 响应对象
//   - 该结构体应该在每个请求处理时创建，确保上下文的正确性
//   - 该结构体支持并发调用，但需要注意请求和响应对象的线程安全性
type Context struct {
	Request  *http.Request       // HTTP 请求对象，包含请求的所有信息
	Response http.ResponseWriter // HTTP 响应对象，用于写入响应数据
}
...
//
// 参数:
//   - contextInterface: 上下文接口，必须为 Context 类型，包含请求和响应对象
//
// 返回值:
...
//	支持类型检查: 使用类型断言检查参数类型，确保参数为 Context 类型
//	支持错误处理: 如果参数类型不正确，会 panic
//	返回新的适配器实例: 返回一个新的适配器实例，包含设置的上下文
//
// 说明:
//
//	该方法使用类型断言检查 contextInterface 参数是否为 Context 类型
...
// 注意事项:
//
//   - contextInterface 参数必须为 Context 类型，否则会 panic
...
func (*Chi) SetContext(contextInterface interface{}) adapter.WebFrameWork {
	...
	if ctx, ok = contextInterface.(Context); !ok { // 如果类型断言失败
		panic("chi 适配器 SetContext: 参数类型错误") // panic，表示参数类型错误
	}
	return &Chi{ctx: ctx} // 返回新的适配器实例，包含设置的上下文
}
...
// 说明:
//
//	该方法调用 Chi 上下文的 Redirect 方法，使用 HTTP 302 状态码
...
func (ch *Chi) Redirect() {
	http.Redirect(ch.ctx.Response, ch.ctx.Request, cfg.Url(cfg.GetLoginUrl()), http.StatusFound) // 调用 http.Redirect 方法，重定向到登录页面
}
...
// 说明:
//
//	该方法调用 Chi 响应头的 Set 方法，设置 Content-Type 头
...
func (ch *Chi) SetContentType() {
	ch.ctx.Response.Header().Set("Content-Type", ch.HTMLContentType()) // 设置响应的 Content-Type 头
}
...
// 说明:
//
//	该方法调用 Chi 响应的 WriteHeader 和 Write 方法
//	首先设置响应状态码为 200（OK）
...
func (ch *Chi) Write(body []byte) {
	ch.ctx.Response.WriteHeader(http.StatusOK) // 设置响应状态码为 200（OK）
	_, _ = ch.ctx.Response.Write(body)         // 写入响应体
}
...
// 说明:
//
//	该方法调用 Chi 请求的 Cookie() 方法，获取 Cookie 值
...
func (ch *Chi) GetCookie() (string, error) {
	cookie, err := ch.ctx.Request.Cookie(ch.CookieKey()) // 获取 Cookie 值
	if err != nil {                                      // 如果获取失败
...
// 说明:
//
//	该方法调用 Chi 请求的 URL.Query().Get() 方法，获取语言参数值
...
func (ch *Chi) Lang() string {
	return ch.ctx.Request.URL.Query().Get("__ga_lang") // 获取语言参数值
}
...
// 说明:
//
//	该方法调用 Chi 请求的 URL.Path 属性，获取请求路径
...
func (ch *Chi) Path() string {
	return ch.ctx.Request.URL.Path // 获取请求路径
}
...
// 说明:
//
//	该方法调用 Chi 请求的 Method 属性，获取 HTTP 方法
...
func (ch *Chi) Method() string {
	return ch.ctx.Request.Method // 获取 HTTP 方法
}
...
// 说明:
//
//	该方法调用 Chi 请求的 ParseMultipartForm 方法，解析表单参数
...
func (ch *Chi) FormParam() url.Values {
	_ = ch.ctx.Request.ParseMultipartForm(32 << 20) // 解析表单参数，最大内存限制为 32MB
	return ch.ctx.Request.PostForm                  // 返回表单参数的键值对集合
}
...
// 说明:
//
//	该方法调用 Chi 请求的 Header.Get() 方法，获取 PJAX 请求头
...
func (ch *Chi) IsPjax() bool {
	return ch.ctx.Request.Header.Get(constant.PjaxHeader) == "true" // 检查 PJAX 请求头是否为 "true"
}
...
// 说明:
//
//	该方法调用 Chi 请求的 URL.Query() 方法，获取查询参数
...
func (ch *Chi) Query() url.Values {
	return ch.ctx.Request.URL.Query() // 获取查询参数
}
...
// 说明:
//
//	该方法调用 Chi 上下文的 Request() 方法，获取原始的 HTTP 请求对象
...
func (ch *Chi) Request() *http.Request {
	return ch.ctx.Request // 调用 Chi 上下文的 Request() 方法，获取原始的 HTTP 请求对象
}
