// Gear 框架适配器包
//
// 本包实现了 GoAdmin 与 Gear Web 框架的集成适配器，使 GoAdmin 管理后台能够在 Gear 框架中运行
// Gear 是一个轻量级、高性能的 Go Web 框架，本适配器实现了 GoAdmin 适配器接口的所有方法
//
// 核心概念:
//
//	适配器模式: Gear 结构体实现了 GoAdmin 的适配器接口，作为 Gear 框架和 GoAdmin 管理后台之间的桥梁
//	上下文转换: 将 Gear 请求上下文（*gear.Context）转换为 GoAdmin 上下文（*context.Context），实现框架无关性
//	路由集成: 将 GoAdmin 的路由注册到 Gear 的路由系统中，支持多种 HTTP 方法和路由参数
//	中间件链: 支持中间件和处理器链，实现请求预处理和后处理
...
// 技术栈:
//
//	Gear: 轻量级、高性能的 Go Web 框架，提供路由、中间件、上下文等功能
...
//
//	import (
//	    "github.com/purpose168/GoAdmin/adapter/gear"
...
//	    // 使用 Gear 适配器
//	    eng := gear.New()
//	    adapter := gear.NewGear()
//	    adapter.SetApp(eng)
//	    adapter.Use(eng, []plugins.Plugin{admin.NewAdmin(generateTables)})
//
//	    // 启动 Gear 服务
//	    app.Listen(":8080")
//	}
...
// 注意事项:
//
//   - Gear 版本要求: 需要使用 Gear 2.x 版本
//   - 路由参数格式: Gear 的路由参数格式为 :param，适配器会自动转换为标准格式 {param}
//   - 表单解析: 表单解析的最大内存限制为 32MB，可以通过修改 32 << 20 调整
...
//   - 响应状态码和响应体: 响应状态码和响应体通过 ctx.End() 方法设置
//   - 请求对象转换: Gear 请求对象（*gear.Context.Req）已经是标准 HTTP 请求对象，无需转换
package gear
...
import (
	...
	"github.com/teambition/gear"                                   // Gear Web 框架，提供路由、中间件、上下文等功能
)

// Gear 结构体实现了 GoAdmin 的适配器接口
// 它作为 Gear 框架和 GoAdmin 管理后台之间的桥梁，实现了适配器接口的所有方法
// 嵌入了 adapter.BaseAdapter 以获得基础适配器功能（如 GetUser、GetUse、GetContent 等）
// ctx 字段存储当前的 Gear 上下文，用于访问请求和响应对象
// app 字段存储 Gear 应用实例，用于注册中间件和处理器
// router 字段存储 Gear 路由器实例，用于注册路由和处理器
type Gear struct {
	adapter.BaseAdapter
	ctx    *gear.Context // Gear 请求上下文，用于访问请求和响应对象
	app    *gear.App     // Gear 应用实例，用于注册中间件和处理器
	router *gear.Router  // Gear 路由器实例，用于注册路由和处理器
}
...
//	适配器注册: 将 Gear 适配器注册到 GoAdmin 引擎中，使 GoAdmin 能够识别和使用 Gear 框架
//	支持多个适配器: GoAdmin 支持同时注册多个适配器，可以在不同框架中使用
//	线程安全: 注册操作是线程安全的，可以在多个 goroutine 中并发调用
//
// 说明:
//
//	调用 engine.Register 方法，将 Gear 适配器注册到 GoAdmin 引擎中
//	注册后，GoAdmin 就能够识别和使用 Gear 框架
//	该函数在包导入时自动执行，无需手动调用
...
func init() {
	engine.Register(new(Gear)) // 注册 Gear 适配器到 GoAdmin 引擎中
}
...
func (gears *Gear) User(ctx interface{}) (models.UserModel, bool) {
	return gears.GetUser(ctx, gears) // 调用基础适配器的 GetUser 方法
}

// Use 实现了 Adapter.Use 方法
// 该方法用于初始化并使用 GoAdmin 插件，将插件注册到 Gear 应用中
//
// 参数:
...
// 功能特性:
//
//	注册插件: 将插件注册到 Gear 应用中，用于扩展 GoAdmin 的功能
//	支持多个插件: 支持同时注册多个插件，实现功能组合
//	支持插件路由注册: 支持插件路由注册，将插件的路由注册到 Gear 的路由系统中
//	支持插件中间件注册: 支持插件中间件注册，将插件的中间件注册到 Gear 的中间件链中
...
func (gears *Gear) Use(app interface{}, plugs []plugins.Plugin) error {
	return gears.GetUse(app, plugs, gears) // 调用基础适配器的 GetUse 方法
}
...
func (gears *Gear) Content(ctx interface{}, getPanelFn types.GetPanelFn, fn context.NodeProcessor, btns ...types.Button) {
	gears.GetContent(ctx, getPanelFn, gears, btns, fn) // 调用基础适配器的 GetContent 方法
}

// HandlerFunc 定义了 Gear 框架的处理器函数类型
// 它接收 Gear 上下文并返回面板和可能的错误，用于自定义处理逻辑
//
// 参数:
//   - ctx: Gear 请求上下文，用于访问请求和响应对象
//
// 返回值:
...
// 说明:
//
//	定义处理函数的签名，接收 Gear 请求上下文，返回面板和错误
...
// 使用示例:
//
//	func myHandler(ctx *gear.Context) (types.Panel, error) {
//	    // 访问数据库
...
//	}
//
//	// 使用 Content 辅助函数创建 Gear 请求处理器
//	app.UseHandler(gear.Content(myHandler))
//
// 注意事项:
//
//   - 处理函数不能为空，必须返回有效的面板
...
// 该中间件将 Gear 的请求处理与 GoAdmin 的内容渲染集成，简化集成过程
//
// 参数:
...
// 功能特性:
//
//	转换 HandlerFunc 为 Middleware: 将 HandlerFunc 类型的处理函数转换为 gear.Middleware 类型的中间件函数
//	简化集成: 简化集成过程，无需手动创建中间件
//	支持在 Gear 路由中使用 GoAdmin 处理函数: 支持在 Gear 路由中使用 GoAdmin 处理函数
//	支持自定义处理逻辑: 支持自定义处理逻辑，实现业务逻辑处理
//
// 说明:
//
//	接收 HandlerFunc 类型的处理函数，返回 gear.Middleware 类型的中间件函数
//	中间件函数内部调用 GoAdmin 的 engine.Content 方法
...
// 该方法用于设置 Gear 应用实例到适配器中，用于后续的路由注册和请求处理
//
// 参数:
...
// 功能特性:
//
//	设置 Gear 应用实例: 将 Gear 应用实例设置到适配器中，用于后续的路由注册和请求处理
//	支持类型检查: 使用类型断言检查参数类型，确保参数类型正确
//	支持错误处理: 如果参数类型错误，返回错误信息
//
// 说明:
//
//	初始化 Gear 路由器，创建新的 Gear 路由器实例
//	使用类型断言检查参数类型，确保参数为 *gear.App 类型
//	如果类型断言失败，返回错误信息
//	如果类型断言成功，将 Gear 应用实例存储到适配器的 app 字段中
//	用于后续的路由注册和请求处理
...
func (gears *Gear) SetApp(app interface{}) error {
	gears.router = gear.NewRouter() // 初始化 Gear 路由器

	var (
		eng *gear.App // Gear 应用实例
		ok  bool      // 类型断言结果
	)

	// 类型断言：验证传入的参数是否为 *gear.App 类型
	// Gear 使用断言来确保类型安全，这是 Go 语言中处理接口的常见模式
	if eng, ok = app.(*gear.App); !ok { // 类型断言
		return errors.New("gear 适配器 SetApp: 参数类型错误") // 返回错误
	}

	gears.app = eng // 保存 Gear 应用实例
	return nil      // 返回 nil，表示设置成功
}

// AddHandler 实现了 Adapter.AddHandler 方法
// 该方法用于向 Gear 应用添加路由处理器，将 GoAdmin 的路由注册到 Gear 的路由系统中
...
// 功能特性:
//
//	注册路由: 将 GoAdmin 的路由注册到 Gear 的路由系统中
...
//	支持上下文转换: 将 Gear 上下文转换为 GoAdmin 上下文，实现框架无关性
//	支持响应头复制: 将 GoAdmin 响应头复制到 Gear 响应中，确保响应头正确传递
//	支持响应体写入: 将 GoAdmin 响应体写入到 Gear 响应中，确保响应体正确传递
//
// 说明:
//
//	如果路由器未初始化，则创建新的路由器
//	使用 Gear 路由器注册路由处理器，调用 router.Handle 方法
//	Gear 使用链式处理器模式，每个处理器可以处理请求并传递给下一个
//	路由处理器接收 Gear 上下文，处理 HTTP 请求和响应
//	首先创建 GoAdmin 上下文，传入 Gear 的请求对象
//	然后使用正则表达式提取路由参数，Gear 的路由参数格式为 :param
//	将路由参数转换为 URL 查询参数，例如：/user/:id → /user?id=123
//	执行处理器链，处理器链包含中间件和处理器
//	将 GoAdmin 响应头复制到 Gear 响应中
//	如果响应体不为空，使用 Gear 的 End 方法结束请求并返回响应
//	否则设置响应状态码
//	最后将路由器作为中间件添加到 Gear 应用中，使 Gear 应用能够处理 GoAdmin 的路由
...
func (gears *Gear) AddHandler(method, path string, handlers context.Handlers) {
	// 如果路由器未初始化，则创建新的路由器
	if gears.router == nil { // 检查路由器是否为空
		gears.router = gear.NewRouter() // 创建新的路由器
	}

	// 使用 Gear 路由器注册路由处理器
	// Gear 使用链式处理器模式，每个处理器可以处理请求并传递给下一个
	gears.router.Handle(strings.ToUpper(method), path, func(c *gear.Context) error { // 注册路由处理器

		// 创建 GoAdmin 上下文，传入 Gear 的请求对象
		ctx := context.NewContext(c.Req) // 创建 GoAdmin 上下文
		...
		for _, param := range params { // 遍历所有路由参数
			...
			if c.Req.URL.RawQuery == "" { // 如果查询字符串为空
				c.Req.URL.RawQuery += p + "=" + c.Param(p) // 添加第一个参数
			} else { // 如果查询字符串不为空
				c.Req.URL.RawQuery += "&" + p + "=" + c.Param(p) // 添加后续参数
			}
		}
		...
		for key, head := range ctx.Response.Header { // 遍历 GoAdmin 响应头
			c.Res.Header().Add(key, head[0]) // 将响应头设置到 Gear 响应中
		}
		...
		if ctx.Response.Body != nil { // 如果 GoAdmin 响应体不为空
			...
			return c.End(ctx.Response.StatusCode, buf.Bytes()) // 结束请求并返回响应
		}

		// 设置响应状态码
		c.Status(ctx.Response.StatusCode) // 设置 Gear 响应的状态码
		return nil                        // 返回 nil，表示处理成功
	})

	// 将路由器作为中间件添加到 Gear 应用中
	// 这样 Gear 应用就能够处理 GoAdmin 的路由
	gears.app.UseHandler(gears.router) // 将路由器作为中间件添加到 Gear 应用中
}
...
// 说明:
//
//	使用类型断言检查参数类型，确保参数为 *gear.Context 类型
//	如果类型断言失败，使用 panic 终止程序，这是 Go 语言中处理严重错误的常见方式
//	如果类型断言成功，创建新的 Gear 适配器实例，设置 Gear 上下文到 ctx 字段中
//	返回新的适配器实例，用于后续的请求处理
//	该方法通常在请求处理中被调用，用于设置请求上下文
//
// 注意事项:
//
//   - contextInterface 参数必须为 *gear.Context 类型，否则会 panic
...
func (*Gear) SetContext(contextInterface interface{}) adapter.WebFrameWork {
	var (
		ctx *gear.Context // Gear 请求上下文
		ok  bool          // 类型断言结果
	)

	// 类型断言：验证传入的参数是否为 *gear.Context 类型
	// 如果类型不匹配，使用 panic 终止程序，这是 Go 语言中处理严重错误的常见方式
	if ctx, ok = contextInterface.(*gear.Context); !ok { // 类型断言
		panic("gear 适配器 SetContext: 参数类型错误") // 终止程序
	}

	// 返回新的 Gear 适配器实例，包含设置的上下文
	return &Gear{ctx: ctx} // 返回新的适配器实例
}
...
// 说明:
//
//	调用 Gear 上下文的 Redirect 方法，使用 HTTP 302 状态码
...
func (gears *Gear) Redirect() {
	gears.ctx.Redirect(config.Url(config.GetLoginUrl())) // 重定向到登录页面
}
...
// 说明:
//
//	调用 Gear 响应头的 Set 方法，设置 Content-Type 头
...
func (gears *Gear) SetContentType() {
	gears.ctx.Res.Header().Set("Content-Type", gears.HTMLContentType()) // 设置 Content-Type 头
}
...
func (gears *Gear) Write(body []byte) {
	gears.ctx.End(http.StatusOK, body) // 结束请求并返回响应
}
...
// 说明:
//
//	调用 Gear 请求的 Cookies.Get 方法，获取指定名称的 Cookie 值
...
func (gears *Gear) GetCookie() (string, error) {
	return gears.ctx.Cookies.Get(gears.CookieKey()) // 获取 Cookie 值
}
...
// 说明:
//
//	调用 Gear 请求的 URL.Query().Get 方法，获取指定名称的查询参数值
...
func (gears *Gear) Lang() string {
	return gears.ctx.Req.URL.Query().Get("__ga_lang") // 获取语言代码
}
...
// 说明:
//
//	调用 Gear 请求的 URL.Path 方法，获取请求路径
//	路径不包含查询参数，只包含路径部分
//	路径以 "/" 开头，如 "/admin/info"
...
func (gears *Gear) Path() string {
	return gears.ctx.Req.URL.Path // 获取请求路径
}
...
// 说明:
//
//	调用 Gear 请求的 Method 方法，获取 HTTP 方法
...
func (gears *Gear) Method() string {
	return gears.ctx.Req.Method // 获取 HTTP 方法
}
...
// 说明:
//
//	调用 Gear 请求的 ParseMultipartForm 方法，解析 multipart/form-data 格式的表单数据
//	32 << 20 表示最大内存限制为 32MB，超过限制的数据会存储到临时文件中
//	返回表单参数的键值对集合，可以通过键获取对应的值
//	支持多值字段，一个键可以对应多个值
//	该方法通常在请求处理中被调用，用于获取表单数据
//
// 注意事项:
//
//   - 会解析表单参数，可能影响性能，建议只调用一次
//   - 表单解析的最大内存限制为 32MB，可以通过修改 32 << 20 调整
//   - 该方法支持并发调用，但需要注意表单参数的线程安全性
//   - 应该只调用一次，多次调用可能导致性能问题
//   - 返回的表单参数可能为空，如果请求体不包含表单数据
func (gears *Gear) FormParam() url.Values {
	_ = gears.ctx.Req.ParseMultipartForm(32 << 20) // 解析 multipart/form-data 格式的表单数据
	return gears.ctx.Req.PostForm                  // 返回表单参数
}
...
// 说明:
//
//	调用 Gear 请求的 Header.Get 方法，获取指定名称的请求头值
...
func (gears *Gear) IsPjax() bool {
	return gears.ctx.Req.Header.Get(constant.PjaxHeader) == "true" // 检查是否为 PJAX 请求
}
...
// 说明:
//
//	调用 Gear 请求的 URL.Query 方法，获取查询参数
...
func (gears *Gear) Query() url.Values {
	return gears.ctx.Req.URL.Query() // 获取查询参数
}
...
// 说明:
//
//	返回 Gear 请求的 Req 字段，该字段已经是标准 HTTP 请求对象
