...
//
// 本包实现了 GoAdmin 与 GoFrame v2 (GF2) Web 框架的集成适配器，使 GoAdmin 管理后台能够在 GoFrame v2 框架中运行
// GoFrame v2 (GF2) 是 GoFrame 框架的第二代版本，提供了更多的功能和更好的性能，本适配器实现了 GoAdmin 适配器接口的所有方法
//
// 核心概念:
//
//	适配器模式: GF2 结构体实现了 GoAdmin 的适配器接口，作为 GoFrame v2 框架和 GoAdmin 管理后台之间的桥梁
//	上下文转换: 将 GoFrame v2 请求上下文（*ghttp.Request）转换为 GoAdmin 上下文（*context.Context），实现框架无关性
//	路由集成: 将 GoAdmin 的路由注册到 GoFrame v2 的路由系统中，支持多种 HTTP 方法和路由参数
...
//	gvar.Var 类型: GoFrame v2 使用 gvar.Var 类型来表示动态变量，需要调用 String() 方法转换为字符串
//
// 技术栈:
//
//	GoFrame v2 (GF2): GoFrame 框架的第二代版本，提供路由、中间件、上下文、gvar.Var 等功能
...
//	    gf2adapter "github.com/purpose168/GoAdmin/adapter/gf2"
...
// 注意事项:
//
//   - GoFrame v2 版本要求: 需要使用 GoFrame v2.x 版本
//   - 路由参数格式: GoFrame v2 的路由参数格式为 :param，适配器会自动转换为标准格式 {param}
//   - 路由绑定格式: GoFrame v2 使用 "METHOD:path" 格式来绑定路由，如 "GET:/admin"
//   - 表单解析: GoFrame v2 的 Form 字段已经包含了解析后的表单参数，无需手动解析
//   - gvar.Var 类型: GoFrame v2 使用 gvar.Var 类型来表示动态变量，需要调用 String() 方法转换为字符串
...
//   - 请求对象转换: GoFrame v2 请求对象（*ghttp.Request.Request）已经是标准 HTTP 请求对象，无需转换
package gf2
...
import (
	...
	"github.com/gogf/gf/v2/net/ghttp"                    // GoFrame v2 HTTP 服务器包，提供路由、中间件、上下文、gvar.Var 等功能
	...
	"github.com/purpose168/GoAdmin/modules/constant"     // GoAdmin 常量模块，提供框架常量定义（如 PjaxHeader、EditPKKey 等）
...
// 它作为 GoFrame v2 (GF2) 框架和 GoAdmin 管理后台之间的桥梁，实现了适配器接口的所有方法
// 嵌入了 adapter.BaseAdapter 以获得基础适配器功能（如 GetUser、GetUse、GetContent 等）
// ctx 字段存储当前的 GoFrame v2 请求上下文，用于访问请求和响应对象
// app 字段存储 GoFrame v2 服务器实例，用于注册路由和处理器
type GF2 struct {
	adapter.BaseAdapter
	ctx *ghttp.Request // GoFrame v2 请求上下文，用于访问请求和响应对象
	app *ghttp.Server  // GoFrame v2 服务器实例，用于注册路由和处理器
}
...
//	适配器注册: 将 GF2 适配器注册到 GoAdmin 引擎中，使 GoAdmin 能够识别和使用 GoFrame v2 框架
//	支持多个适配器: GoAdmin 支持同时注册多个适配器，可以在不同框架中使用
//	线程安全: 注册操作是线程安全的，可以在多个 goroutine 中并发调用
//
// 说明:
//
//	调用 engine.Register 方法，将 GF2 适配器注册到 GoAdmin 引擎中
//	注册后，GoAdmin 就能够识别和使用 GoFrame v2 框架
//	该函数在包导入时自动执行，无需手动调用
...
func init() {
	engine.Register(new(GF2)) // 注册 GF2 适配器到 GoAdmin 引擎中
}
...
// 该方法用于初始化并使用 GoAdmin 插件，将插件注册到 GoFrame v2 应用中
//
// 参数:
//   - app: Web 应用实例，实际类型为 *ghttp.Server
//   - plugins: 插件列表，包含要加载的 GoAdmin 插件（如 Admin 插件）
//
// 返回值:
...
// 功能特性:
//
//	注册插件: 将插件注册到 GoFrame v2 应用中，用于扩展 GoAdmin 的功能
//	支持多个插件: 支持同时注册多个插件，实现功能组合
//	支持插件路由注册: 支持插件路由注册，将插件的路由注册到 GoFrame v2 的路由系统中
//	支持插件中间件注册: 支持插件中间件注册，将插件的中间件注册到 GoFrame v2 的处理器链中
...
//	插件会注册自己的路由和中间件到 GoFrame v2 应用中
...
func (gf2 *GF2) Content(ctx interface{}, getPanelFn types.GetPanelFn, fn context.NodeProcessor, btns ...types.Button) {
	gf2.GetContent(ctx, getPanelFn, gf2, btns, fn) // 调用基础适配器的 GetContent 方法
}
...
func (gf2 *GF2) User(ctx interface{}) (models.UserModel, bool) {
	return gf2.GetUser(ctx, gf2) // 调用基础适配器的 GetUser 方法
}

// AddHandler 实现了 Adapter.AddHandler 方法
// 该方法用于向 GoFrame v2 应用添加路由处理器，将 GoAdmin 的路由注册到 GoFrame v2 的路由系统中
// GoFrame v2 的路由参数格式为 :param，需要转换为标准 URL 查询参数格式 ?param=value
// GoFrame v2 使用 "METHOD:path" 格式来绑定路由，如 "GET:/admin"
//
// 参数:
...
// 功能特性:
//
//	注册路由: 将 GoAdmin 的路由注册到 GoFrame v2 的路由系统中
...
//	支持上下文转换: 将 GoFrame v2 上下文转换为 GoAdmin 上下文，实现框架无关性
//	支持响应头复制: 将 GoAdmin 响应头复制到 GoFrame v2 响应中，确保响应头正确传递
//	支持响应体写入: 将 GoAdmin 响应体写入到 GoFrame v2 响应中，确保响应体正确传递
//
// 说明:
//
//	使用 GoFrame v2 的 BindHandler 方法绑定路由处理器，GoFrame v2 使用 "METHOD:path" 格式来绑定路由
//	路由处理器接收 GoFrame v2 请求对象，处理 HTTP 请求和响应
//	首先创建 GoAdmin 上下文，传入 GoFrame v2 的标准 HTTP 请求对象
//	然后使用正则表达式提取路由参数，GoFrame v2 的路由参数格式为 :param
//	将路由参数转换为 URL 查询参数，例如：/user/:id → /user?id=123
//	执行处理器链，处理器链包含中间件和处理器
//	将 GoAdmin 响应头复制到 GoFrame v2 响应中
//	如果响应体不为空，使用 GoFrame v2 的 WriteStatus 方法写入响应状态码和响应体
//	否则只写入响应状态码
...
func (gf2 *GF2) AddHandler(method, path string, handlers context.Handlers) {
	...
	gf2.app.BindHandler(strings.ToUpper(method)+":"+path, func(c *ghttp.Request) { // 绑定路由处理器
		...
		for _, param := range params { // 遍历所有路由参数
			...
			if c.Request.URL.RawQuery == "" { // 如果查询字符串为空
				c.Request.URL.RawQuery += p + "=" + c.GetRequest(p).String() // 添加第一个参数
			} else { // 如果查询字符串不为空
				c.Request.URL.RawQuery += "&" + p + "=" + c.GetRequest(p).String() // 添加后续参数
			}
		}
		...
		for key, head := range ctx.Response.Header { // 遍历 GoAdmin 响应头
			c.Response.Header().Add(key, head[0]) // 将响应头设置到 GoFrame v2 响应中
		}
...
// 该方法用于设置 GoFrame v2 服务器实例到适配器中，用于后续的路由注册和请求处理
//
// 参数:
...
// 功能特性:
//
//	设置 GoFrame v2 服务器实例: 将 GoFrame v2 服务器实例设置到适配器中，用于后续的路由注册和请求处理
...
//	如果类型断言失败，返回错误信息
//	如果类型断言成功，将 GoFrame v2 服务器实例存储到适配器的 app 字段中
//	用于后续的路由注册和请求处理
...
func (gf2 *GF2) SetApp(app interface{}) error {
	var (
		eng *ghttp.Server // GoFrame v2 服务器实例
		ok  bool          // 类型断言结果
	)
	...
	if eng, ok = app.(*ghttp.Server); !ok { // 类型断言
		return errors.New("gf2 适配器 SetApp: 参数类型错误") // 返回错误
	}

	gf2.app = eng // 保存 GoFrame v2 服务器实例
...
//	如果类型断言成功，创建新的 GF2 适配器实例，设置 GoFrame v2 请求上下文到 ctx 字段中
//	返回新的适配器实例，用于后续的请求处理
...
func (*GF2) SetContext(contextInterface interface{}) adapter.WebFrameWork {
	var (
		ctx *ghttp.Request // GoFrame v2 请求上下文
		ok  bool           // 类型断言结果
	)
	...
	if ctx, ok = contextInterface.(*ghttp.Request); !ok { // 类型断言
		panic("gf2 适配器 SetContext: 参数类型错误") // 终止程序
	}

	// 返回新的 GF2 适配器实例，包含设置的上下文
	return &GF2{ctx: ctx} // 返回新的适配器实例
}
...
//	gvar.Var 类型: GoFrame v2 的 Cookie.Get 返回的是 gvar.Var 类型，需要调用 String() 方法转换为字符串
//
// 说明:
//
//	调用 GoFrame v2 请求的 Cookie.Get 方法，获取指定名称的 Cookie 值
//	Cookie 键名通过 CookieKey() 方法获取，默认为 "goadmin_session"
//	返回的 Cookie 值是 gvar.Var 类型，需要调用 String() 方法转换为字符串
//	返回 Cookie 值和 nil 错误
...
//   - GoFrame v2 的 Cookie.Get 返回的是 gvar.Var 类型，需要调用 String() 方法转换为字符串
func (gf2 *GF2) GetCookie() (string, error) {
	return gf2.ctx.Cookie.Get(gf2.CookieKey()).String(), nil // 获取 Cookie 值并转换为字符串
}
...
// 说明:
//
//	调用 GoFrame v2 请求的 URL.Query().Get 方法，获取指定名称的查询参数值
...
func (gf2 *GF2) Lang() string {
	return gf2.ctx.Request.URL.Query().Get("__ga_lang") // 获取语言代码
}
...
// 说明:
//
//	调用 GoFrame v2 请求的 URL.Path 方法，获取请求路径
//	路径不包含查询参数，只包含路径部分
//	路径以 "/" 开头，如 "/admin/info"
...
func (gf2 *GF2) Path() string {
	return gf2.ctx.URL.Path // 获取请求路径
}
...
// 说明:
//
//	调用 GoFrame v2 请求的 Method 方法，获取 HTTP 方法
...
func (gf2 *GF2) Method() string {
	return gf2.ctx.Method // 获取 HTTP 方法
}
...
//	自动解析: GoFrame v2 的 Form 字段已经包含了解析后的表单参数，无需手动解析
//
// 说明:
//
//	返回 GoFrame v2 请求的 Form 字段，该字段已经包含了解析后的表单参数
...
//   - GoFrame v2 的 Form 字段已经自动解析，无需手动调用 ParseForm 或 ParseMultipartForm
//   - 应该在数据查询时调用，确保表单参数正确
func (gf2 *GF2) FormParam() url.Values {
	return gf2.ctx.Form // 返回表单参数
}
...
// 说明:
//
//	调用 GoFrame v2 请求的 URL.Query 方法，获取查询参数
...
func (gf2 *GF2) Query() url.Values {
	return gf2.ctx.Request.URL.Query() // 获取查询参数
}
...
// 说明:
//
//	调用 GoFrame v2 请求的 Header.Get 方法，获取指定名称的请求头值
...
func (gf2 *GF2) IsPjax() bool {
	return gf2.ctx.Header.Get(constant.PjaxHeader) == "true" // 检查是否为 PJAX 请求
}
...
// 说明:
//
//	调用 GoFrame v2 请求的 Response.RedirectTo 方法，使用 HTTP 302 状态码
...
func (gf2 *GF2) Redirect() {
	gf2.ctx.Response.RedirectTo(config.Url(config.GetLoginUrl())) // 重定向到登录页面
}
...
// 说明:
//
//	调用 GoFrame v2 响应头的 Add 方法，设置 Content-Type 头
...
func (gf2 *GF2) SetContentType() {
	gf2.ctx.Response.Header().Add("Content-Type", gf2.HTMLContentType()) // 设置 Content-Type 头
}
...
// 说明:
//
//	调用 GoFrame v2 响应的 WriteStatus 方法，结束请求并返回响应
...
func (gf2 *GF2) Write(body []byte) {
	gf2.ctx.Response.WriteStatus(http.StatusOK, body) // 写入响应状态码和响应体
}
...
// 说明:
//
//	返回 GoFrame v2 请求的 Request 字段，该字段已经是标准 HTTP 请求对象
