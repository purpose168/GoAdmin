// buffalo.go - Buffalo 框架适配器文件
// 包名：buffalo
// 作者：GoAdmin 团队
// 创建日期：2020年
// 描述：本文件提供 GoAdmin 在 Buffalo Web 框架下的适配器实现
...
import (
	...
	// 导入 Buffalo 适配器，用于将 GoAdmin 集成到 Buffalo 框架
	...
	// 导入 AdminLTE UI 主题，提供后台管理界面主题
	// 使用空白导入（_）触发主题的 init() 函数
	...
	"github.com/gobuffalo/buffalo"                  // Buffalo Web 框架包，提供 HTTP 服务器和路由功能
...
//
// 技术细节：
//   - 使用 buffalo.New() 创建 Buffalo 服务器实例
...
//   - 使用 eng.Use() 将引擎应用到 Buffalo 服务器
//   - 使用 template.AddComp() 添加模板组件
//   - 使用 eng.HTML() 注册 HTML 路由
//   - 使用 bu.ServeFiles() 配置静态文件服务
//
// 使用场景：
//   - 集成测试
//   - 单元测试
...
func internalHandler() http.Handler {
	// 创建 Buffalo HTTP 服务器实例，设置环境为测试模式
	bu := buffalo.New(buffalo.Options{
		Env:  "test",           // 设置环境为测试模式
		Addr: "127.0.0.1:9033", // 设置监听地址和端口
	}) // 创建 Buffalo 服务器实例
	...
	// 配置静态文件服务，用于提供上传文件的访问
	bu.ServeFiles("/uploads", http.Dir("./uploads")) // 配置静态文件服务
